...
#'     best response correspondences. The default value is \code{0.1}. The
#'     smaller the value is, the smoother the correspondence curves are.
...
xs <- ys <- xe <- ye <- text <- NULL
  ...
  f1 <- p1 %>%
    stringr::str_replace_all(pattern = pars[1],
                             replacement = "FIRST_PARAM") %>%
    stringr::str_replace_all(pattern = pars[2],
  ...
  if (sol_type == 2) {
    ...
    for (i in seq_along(par1_seq)) {
      fn2 <- function(y) {
        -1 * eval(f2[[1]], envir = list(x = par1_seq[i]))
      }
    ...
    for (i in 1:nrow(df_p1_br)) {
      for (j in 1:nrow(df_p2_br))
        if (abs(df_p1_br$x[i] - df_p2_br$x[j]) < delta &
            abs(df_p1_br$y[i] - df_p2_br$y[j]) < delta) {
