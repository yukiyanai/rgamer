#' @title Plays a normal-form game by simulation (best response)
  ...
  if (game$type == "matrix") {
    ...
    for (i in 2:n_periods) {
      ...
      if (runif(1) < rho) {
        ...
        play1[i] <- try(uniroot(fd1, interval = s1)$root,
                        silent = TRUE)
      ...
      if (runif(1) < rho) {
        ...
        play2[i] <- try(uniroot(fd2, interval = s2)$root,
                        silent = TRUE)
    ...
    for (i in 2:n_periods) {
      ...
      if (runif(1) < rho) {
        ...
        play1[i] <- try(optim(par = median(s1),
                              fn = f1,
                              method = "L-BFGS-B",
                              lower = s1[1],
                              upper = s1[2],
      ...
      if (runif(1) < rho) {
        ...
        play2[i] <- try(optim(par = median(s2),
                              fn = f2,
                              method = "L-BFGS-B",
                              lower = s2[1],
                              upper = s2[2],
